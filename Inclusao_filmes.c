...
void salvar_filmes(Filme f) {
    // Adiciona no array de filmes em mem�ria
    if (n_filmes < MAX_FILMES) {
        filmes[n_filmes++] = f;
    }
    else {
        printf("Limite m�ximo de filmes atingido!\n");
        return;
    }
    ...
    // Salva os dados no arquivo em formato texto, separados por v�rgula
    ...
    if (confirma == 'S') {
        ...
        if (arquivo != NULL) {
            while (fgets(linha, sizeof(linha), arquivo)) {
                ...
                // Verifica se j� existe c�digo igual
