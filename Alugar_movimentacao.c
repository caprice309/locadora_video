...
void alugar_movimentacao() {
    ...
    while (1) {
        ...
        gotoxy(7, 7);  printf("ALUGAR FILME");
        ...
        if (quantidadeAlugada >= quantidadeTotal) {
            gotoxy(2, 23); printf("NAO HA MAIS UNIDADES DISPONIVEIS PARA ALUGAR!");
            getch();
        ...
        if (!arqTipos) {
            gotoxy(2, 23); printf("ERRO AO ABRIR ARQUIVO DE TIPOS!");
            getch();
            continue;
        }
        ...
        if (!tipoEncontrado) {
            gotoxy(2, 23); printf("TIPO DE FILME INVALIDO!");
            getch();
            if (!perguntarNovoEmprestimo()) {
                menu_movimentacao();
                return;
        ...
        if (!arqClientes) {
            gotoxy(2, 23); printf("ERRO AO ABRIR ARQUIVO DE CLIENTES!");
            getch();
            continue;
        }
        ...
        if (!clienteEncontrado) {
            gotoxy(2, 23); printf("CLIENTE NAO ENCONTRADO OU NOME NAO CORRESPONDE!");
            getch();
            if (!perguntarNovoEmprestimo()) {
                menu_movimentacao();
                return;
        ...
        do {
            ...
            printf("                                   ");
            gotoxy(2, 23);
            printf("CONFIRMA EMPRESTIMO ( S/N ) ? ");
        ...
        // ---- ATUALIZAR ARQUIVO (marcar como alugado) ----
        FILE* arqUpdate = fopen("filmes.txt", "r+");
        if (!arqUpdate) {
            gotoxy(2, 23); printf("ERRO AO ATUALIZAR ARQUIVO!");
            getch();
            if (!perguntarNovoEmprestimo()) {
                menu_movimentacao();
                return;
        ...
        strcpy(h.tituloFilme, filmes[indiceFilme].titulo);
        ...
        for (int k = 0; k < n_historico; k++) {
            if (strcmp(historico[k].codigoFilme, h.codigoFilme) == 0) {
                h.vezesAlugadoFilme = historico[k].vezesAlugadoFilme + 1;
                achouFilme = 1;
            }
            if (strcmp(historico[k].codigoCliente, h.codigoCliente) == 0) {
                h.vezesAlugouCliente = historico[k].vezesAlugouCliente + 1;
                achouCliente = 1;
            }
        }

        if (!achouFilme) h.vezesAlugadoFilme = 1;
        if (!achouCliente) h.vezesAlugouCliente = 1;

        // ===== DATA DO ALUGUEL =====
        time_t agora = time(NULL);
        struct tm dataStruct;
        struct tm* dataPtr = localtime(&agora);
        if (dataPtr != NULL) {
            dataStruct = *dataPtr; // copia para stack local
            strftime(h.dataAluguel, sizeof(h.dataAluguel), "%d/%m/%Y", &dataStruct);
        }
        else {
            strncpy(h.dataAluguel, "00/00/0000", sizeof(h.dataAluguel) - 1);
        ...
        if (dataPtr != NULL) {
            dataStruct = *dataPtr;
            strftime(h.dataDevolucao, sizeof(h.dataDevolucao), "%d/%m/%Y", &dataStruct);
        }
        else {
            strncpy(h.dataDevolucao, "00/00/0000", sizeof(h.dataDevolucao) - 1);
            h.dataDevolucao[sizeof(h.dataDevolucao) - 1] = '\0';
        }

        salvar_historico(h);

        // ===== PERGUNTAR NOVO EMPRESTIMO =====
        if (!perguntarNovoEmprestimo()) {
            menu_movimentacao();
            return;
        }
