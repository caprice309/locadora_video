...
int linha_vazia(const char* s) {
    while (*s != '\0') {
        if (*s != ' ' && *s != '\n' && *s != '\r' && *s != '\t')
            return 0; // achou caractere ent�o n�o est� vazia
        s++;
    }
    return 1; // s� tinha espa�os/brancos ent�o vazia :D
}
...
void balanco_movimentacoes() {
    ...
    while (1) {
        ...
        // ====== M�DIA + MLEHORES ======
        char clientes[2000][20];        // c�digo do cliente
        char nomeClientes[2000][100];   // nome do cliente
        int locacoesCliente[2000];      // quantidade de loca��es
